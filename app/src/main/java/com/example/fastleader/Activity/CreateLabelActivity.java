...
public class CreateLabelActivity extends AppCompatActivity {
    ...
    private void UploadDataToFirebase(String value) {
        ...
        Query query = FirebaseDatabase.getInstance().getReference("Labels");

        query.orderByChild("L_NewLabelName").equalTo(SelectLabel.getSelectedItem().toString())
                .addValueEventListener(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) {

                        for (DataSnapshot dataSnapshot : snapshot.getChildren()){
                            String L_ID = dataSnapshot.child("L_ID").getValue().toString();
